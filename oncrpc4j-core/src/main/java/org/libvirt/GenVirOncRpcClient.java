...
public class GenVirOncRpcClient extends GenAbstractOncRpcClient<GenVirOncRpcSvc> implements GenItfOncRpcClient<GenVirOncRpcSvc> {
    ...
    public GenVirOncRpcClient(InetAddress address, int port) {
        this(address,port,0,IoStrategy.SAME_THREAD,null);
    }
    public GenVirOncRpcClient(InetAddress host,  int port, int localPort, IoStrategy ioStrategy, String serviceName) {
