...
/**
 * 
 * Constructeur compliqué, tenter de le simplifer (sans générique)
 * 
 * @author jmk
 *
 */
...
public class GenericPortmapClient implements OncPortmapClient{
    ...
    public GenericPortmapClient(XdrTransportItf<OncRpcSvc> transport) throws RpcProgUnavailable {

       OncPortmapClient portmapClient = new GenRpcbindV4Client(IRpcCall.getImpl(100000, 4, _auth, transport));
        if( !portmapClient.ping() ) {
            portmapClient = new GenPortmapV2Client2(  IRpcCall.getImpl(100000, 2, _auth, transport) );
            if(!portmapClient.ping()) {
    ...
    public static void main(String[] args) throws InterruptedException, IOException, OncRpcException, TimeoutException {
        ...
        IOncRpcClient rpcClient =  IOncRpcClient.getImpl(InetAddress.getByName(null), IpProtocolType.UDP, 111);
