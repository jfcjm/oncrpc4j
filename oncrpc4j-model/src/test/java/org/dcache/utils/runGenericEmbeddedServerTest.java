...
public abstract class runGenericEmbeddedServerTest
    ...
    private EmbeddedGenericServerFactory<SVC_T,CALL_T,BUILDER_T,TRANSPORT_T,REPLY_T> _factory;

    runGenericEmbeddedServerTest(EmbeddedGenericServerFactory<SVC_T,CALL_T,BUILDER_T,TRANSPORT_T,REPLY_T> factory)  {
        _factory = factory;
    }
    
    protected  EmbeddedGenericServer<SVC_T,CALL_T,BUILDER_T,TRANSPORT_T,REPLY_T> createEmbeddedServer(int port) throws IOException {
    ...
    public void testProc0() throws IOException {
        try ( EmbeddedGenericServer<SVC_T,CALL_T,BUILDER_T,TRANSPORT_T,REPLY_T> srv = createEmbeddedServer (0)){
            assertTrue("Server should listen on a port != 0",srv.getListeningPort()>0);
            System.out.println(srv.getListeningPort());
            RpcCallItf<SVC_T,CALL_T,TRANSPORT_T,REPLY_T> caller = srv.getClientCall();
    ...
    public void testVersionUnavailable() throws IOException {
        try (EmbeddedGenericServer<SVC_T,CALL_T,BUILDER_T,TRANSPORT_T,REPLY_T>  srv = createEmbeddedServer (0)){
            assertTrue("Server should listen on a port != 0",srv.getListeningPort()>0);
            System.out.println(srv.getListeningPort());
            RpcCallItf<SVC_T,CALL_T,TRANSPORT_T,REPLY_T> caller = srv.getBadVersionClientCall();
    ...
    public void testProgUnavailable() throws IOException {
        try (EmbeddedGenericServer<SVC_T,CALL_T,BUILDER_T,TRANSPORT_T,REPLY_T>  srv = createEmbeddedServer (0)){
            assertTrue("Server should listen on a port != 0",srv.getListeningPort()>0);
            System.out.println(srv.getListeningPort());
            RpcCallItf<SVC_T,CALL_T,TRANSPORT_T,REPLY_T> caller = srv.getBadProgClientCall();
