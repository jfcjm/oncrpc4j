...
public class ClientServerTVirRpcest {
    ...
    private RpcSvcItf<VirRpcSvc> clnt;
    private IVirRpcCall clntCall;

    @Before
    public void setUp() throws IOException {

        RpcDispatchable<VirRpcSvc> echo = ( aCall) -> {
            ...
            switch (call.getProcedure()) {
                ...
                case UPPER: {
                    IVirRpcCall cb = IVirRpcCall.getImpl(PROGNUM, PROGVER, null, call.getTransport());
                    XdrString s = new XdrString();
                    call.retrieveCall(s);
                    assertEquals("hello",s.stringValue());
                    cb.call(ECHO, s, s);
        ...
        RpcDispatchable<VirRpcSvc> upper = ( call) -> {
            XdrString s = new XdrString();
            call.retrieveCall(s);
        ...
        clntCall = IVirRpcCall.getImpl(PROGNUM, PROGVER, null, t);
    }
    ...
    public void shouldTriggerClientCallbackEvenIfOtherClientDisconnected() throws IOException {
        ...
        VirRpcSvc clnt2 =  VirRpcSvcBuilder.getImpl()
                .withTCP()
                .withClientMode()
                .withWorkerThreadIoStrategy()
                .build();
