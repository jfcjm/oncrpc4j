...
public final class VirRpcProtocolFilterImpl  extends AbstractRpcProtocolFilter<VirRpcSvc> implements VirRpcProtcolFilter {

    private final static Logger _log = LoggerFactory.getLogger(VirRpcProtocolFilterImpl.class);

    public VirRpcProtocolFilterImpl(ReplyQueueItf<VirRpcSvc> replyQueue) {
        super(replyQueue);
    }
    ...
    public NextAction handleRead(FilterChainContext ctx) throws IOException {
         ...
         * request was coming.
         */
        XdrTransportItf<VirRpcSvc> transport = new AbstractGrizzlyXdrTransport<>(ctx.getConnection(), (InetSocketAddress)ctx.getAddress(), _replyQueue);
        switch (message.type()) {
            ...
            case RpcMessageType.REPLY:
                ...
                try {
                    final RpcReplyItf<VirRpcSvc> reply = createReply(xdr, message, transport);
                    _log.debug("Rpc reply is {}",reply);
                       CompletionHandler<RpcReplyItf<VirRpcSvc>, XdrTransportItf<VirRpcSvc>> callback = _replyQueue.get(message.xid());
                    if (callback != null) {
    ...
    @Override
    protected RpcReplyItf<VirRpcSvc> createReply(Xdr xdr, RpcMessage message,
            XdrTransportItf<VirRpcSvc> transport) throws OncRpcException, IOException {
        return   VirRpcReply.getImpl(message.xid(), xdr, transport);
    }
}
